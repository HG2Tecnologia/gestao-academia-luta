/**
 * Limpa as modalidades/faixas incompletas da Forny School
 * e repopula com as 8 modalidades padrão.
 */
...
const ACADEMIA_ID = 'NfJLlek5nNTDhr9iW82G'; // Forny School
...
async function deletarColecao(colRef) {
...
async function main() {
  console.log(`\n🔥 Repopulando Forny School (${ACADEMIA_ID})\n`);

  const modCol = db.collection('academias').doc(ACADEMIA_ID).collection('modalidades');
  const faixaCol = db.collection('academias').doc(ACADEMIA_ID).collection('faixas');

  const modApagadas = await deletarColecao(modCol);
  const faixasApagadas = await deletarColecao(faixaCol);
  if (modApagadas > 0 || faixasApagadas > 0) {
    console.log(`🗑  Removidas ${modApagadas} modalidades e ${faixasApagadas} faixas antigas.\n`);
  }

  let totalMod = 0, totalFaixas = 0;
  for (const mod of MODALIDADES_DEFAULT) {
    const modRef = modCol.doc();
    const modId = modRef.id;
    await modRef.set({
      id: modId, nome: mod.nome, cor: mod.cor, ativo: true,
      criado_em: admin.firestore.FieldValue.serverTimestamp(),
    });
    totalMod++;
    for (const f of mod.faixas) {
      const fRef = faixaCol.doc();
      await fRef.set({
        id: fRef.id, modalidadeId: modId, nome: f.nome, cor: f.cor,
        ordem: f.ordem, tem_graus: f.tem_graus, max_graus: f.max_graus,
        criado_em: admin.firestore.FieldValue.serverTimestamp(),
      });
      totalFaixas++;
    }
    console.log(`  ✅ ${mod.nome} — ${mod.faixas.length} faixas`);
  }

  console.log(`\n✨ ${totalMod} modalidades e ${totalFaixas} faixas criadas!\n`);
}
