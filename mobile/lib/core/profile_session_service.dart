...
abstract class ProfileSessionService {
  ...
  static Future<StoredUser?> refresh() async {
    ...
    final id = selecionado?['usuarioId']?.toString() ??
        remoto['usuarioId']?.toString() ??
        firebaseUser.uid;
    final academiaId = selecionado?['academiaId']?.toString() ??
        remoto['academiaId']?.toString();
    final perfil = selecionado?['perfil_nome']?.toString() ??
        remoto['perfil']?.toString() ??
        'Aluno';
    final colecao = selecionado?['colecao']?.toString() ??
